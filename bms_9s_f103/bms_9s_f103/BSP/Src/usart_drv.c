...
{
    if (id != USART_ID_1) { return USART_ERROR; }

    HAL_StatusTypeDef status = HAL_UART_Transmit(&huart1, &byte, 1U, USART_TX_TIMEOUT_MS);
...
{
    if (data == NULL || len == 0U || id != USART_ID_1) {
        return USART_ERROR;
    }

    HAL_StatusTypeDef status = HAL_UART_Transmit(&huart1, (uint8_t *)data, len, USART_TX_TIMEOUT_MS);
...
{
    if (byte == NULL || id != USART_ID_1) {
        return USART_ERROR;
    }

    if (ring_buf_get(&usart1_rb, byte) != 0U) {
...
{
    if (id != USART_ID_1) { return 0U; }
    return ring_buf_available(&usart1_rb);
}

void usart_flush_rx(usart_id_t id)
{
    if (id == USART_ID_1) { ring_buf_flush(&usart1_rb); }
}
...
{
    return (id == USART_ID_1) ? &huart1 : NULL;
}
